...
function selectBoxDynamic($field_name = '', $tableName = 'ia_setting', $colom = 'value', $selected = '', $attr = '', $whereCol = '', $whereVal = '')
{
    ...
    if ($query->num_rows() > 0) {
        ...
        foreach ($catlog_data as $catlogData) {
            ...
            if (!isset($catlogData->$tab_id)) {
                $tab_id = "id";
                // print_r($catlogData->$tab_id);die();
            }
            // if ($tableName == 'task_status') {
            //     print_r($catlogData);die();
            // }
...
function customMenus()
{
    ...
    foreach ($menus as $key => $value) {
        ...
        $pId = $value->id;
        $subMenus = $CI->db->query('SELECT * FROM ia_menu WHERE parent_id = '.$pId.'')->result();
        if (!empty($subMenus)) {
            
            foreach ($subMenus as $subkey => $subvalue) {
                $route_class = str_replace($slug, '', $subvalue->slug);
                $subcl = 'not-active';
                if ($CI->router->method == $route_class) {
                    $subcl = 'active';
                }
