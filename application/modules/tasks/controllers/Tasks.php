...
class Tasks extends CI_Controller {
    ...
    public function index() { 
        if ($this->input->is_ajax_request()) {
            ...
            if ($this->user_type == "admin") {
                echo json_encode($this->Tasks_model->getTaskItems($this->input->post('status'), $this->input->post('todo_page')));
                exit;
            }
            else {
                if (CheckPermission("todo", "all_read")) {
                    ...
                    echo json_encode($this->Tasks_model->getUserTodoItems($this->input->post('status'), $this->input->post('todo_page')));
                    exit;
                }
            }
        ...
        if(CheckPermission("ticket", "all_read,own_read")){
            $con = '';
            $data["view_data"]= $this->Tasks_model->getData($con);
            ...
            $data["obj"] = $this;
            $data['total_pages_finished'] = ceil($this->totalRows('tasks', array(
                'status' => "to_do",
                'assigned_to' => $this->user_id,
            )) / 10);
            $data['total_pages_unfinished'] = ceil($this->totalRows('tasks', array(
                'status' => "done",
                'assigned_to' => $this->user_id,
            )) / 10);
            $data['total_pages_process'] = ceil($this->totalRows('tasks', array(
                'status' => "in_progress",
                'assigned_to' => $this->user_id,
            )) / 10);
    ...
    public function updateTaskItemsOrder()
    {
        if ($this->input->post()) {
            $this->Tasks_model->updateTaskItemsOrder($this->input->post());
        }
    }
    ...
    {
        $success = $this->Tasks_model->changeTaskStatus($id, $status);
        $res = 0;
        if ($success) {
            $res = 1;
 ...
 public function checkEnableStatus()
    ...
    public function view($id) {
        $tk_data = $this->Tasks_model->getDataBy('ia_ticket', $id, 'ticket_id');
        $comm_data = $this->Tasks_model->getDataBy('ia_ticket_comments', $id, 'ticket_id');
        /*echo '<pre>';
        print_r($tk_data);
    ...
    public function getUserDetails($id) {
        return $this->Tasks_model->getDataBy('ia_users', $id, 'ia_users_id');
    ...
    public function saveComment() {
        ...
        $this->Tasks_model->insertRow('ia_ticket_comments', $data);
        $tk_data = $this->Tasks_model->getDataBy('ia_ticket', $this->input->post('ticket_id'), 'ticket_id');
        if($tk_data[0]->ticket_status != 'Close') {
            $this->Tasks_model->updateRow('ia_ticket', 'ticket_id', $this->input->post('ticket_id'), array('ticket_status' => 'Ongoing'));
        }
    ...
    {
        $success = $this->Tasks_model->changeStatus($id, $status);
        $res = 0;
        if ($success) {
            $res = 1;
    ...
    public function closeTicket(){
        $tk_data = $this->Tasks_model->getDataBy('ia_ticket', $this->input->post('id'), 'ticket_id');
        $users = array(
            $tk_data[0]->user_id,
        ...
        foreach ($users as $ukey => $uvalue) {
            $user_detail = $this->Tasks_model->getDataBy('ia_users', $uvalue, 'ia_users_id');
        ...
        //$time = $this->Tasks_model->getQrResult("SELECT NOW() AS `mka_time`");
        $data_arr = array(  
        ...
        $this->Tasks_model->updateRow('ia_ticket', 'ticket_id', $this->input->post('id'), $data_arr);
        echo 1;
        exit;
    }

    public function getusetnamebyid($id) {
        $ur_data = $this->Tasks_model->getDataBy('ia_users', $id, 'ia_users_id');
        $res = '';
        ...
        if($this->session->get_userdata()['user_details'][0]->user_type == 'admin') {
            $html .= '<div class="col-md-3">
                    <div class="form-group form-float">
                        <div class="form-line">';
                $html .='<select name="tasks___tasks_assigned_to___filter" class="filter-field form-control">';
                    $html .='<option value="all">'.lang('all').'</option>';
            $sel = ''; if(isset($filter_coo->tasks___tasks_assigned_to___filter)) { $sel = $filter_coo->tasks___tasks_assigned_to___filter; }  $html .= $this->getFilterDropdownOptions('ticket_client', 'ia_users', 'name', ''.$sel.'');
            $html .= '</select>
                            <label class="form-label" style="margin-top: -10px;">Assigned To</label>
                        </div>
                    </div>
                </div>';
        }
...
';
    ...
    public function getFilterDropdownOptions($field, $rel_table, $rel_col, $selected) {
        $res = $this->Tasks_model->getFilterDropdownOptions($field, $rel_table, $rel_col);
        $option = '';
    ...
    public function addEdit() { 
        ...
        if($this->input->post('id')) {
            $return = $this->Tasks_model->updateRow('tasks', 'id', $this->input->post('id'), $data);
            // $this->sendMailTicket($this->input->post('id'), $this->input->post('ticket_description'));
            echo json_encode($return);
            exit;
        } else { 
            // $data['created'] = date('Y-m-d H:i:s');
            $last_id = $this->Tasks_model->insertRow('tasks', $data);
    ...
    public function getModal() {
        if($this->input->post('id')){
            $data['data']= $this->Tasks_model->getDataId($this->input->post('id'));
    ...
    public function delete($ids) {
        ...
        foreach ($idsArr as $key => $value) {
            $this->Tasks_model->deleteData($value);        
        }
    ...
    public function deleteData($id) { 
        $this->Tasks_model->deleteData($id);
    ...
    public function ajxData(){
        $primaryKey = 'id';
        $table      = 'tasks';
        $joinQuery  =  "FROM tasks LEFT JOIN  `ia_users` AS `users0` ON (`users0`.`user_id` = `tasks`.`assigned_to`)
";
    $columns = array();
    array_push($columns, array( 'db' => '`tasks`.`id` AS `task_id`', 'dt' => count($columns), 'field' => 'task_id'    ));
    array_push($columns, array( 'db' => '`tasks`.`title` AS `task_title`', 'dt' => count($columns), 'field' => 'task_title' ));
    array_push($columns, array( 'db' => '`tasks`.`start_date` AS `start_date`', 'dt' => count($columns), 'field' => 'start_date' ));
    array_push($columns, array( 'db' => '`tasks`.`deadline` AS `deadline`', 'dt' => count($columns), 'field' => 'deadline' ));
    array_push($columns, array( 'db' => '`tasks`.`project_id` AS `project_id`', 'dt' => count($columns), 'field' => 'project_id' ));
    array_push($columns, array( 'db' => '`users0`.`name` AS `name`', 'dt' => count($columns), 'field' => 'name' ));
    array_push($columns, array( 'db' => '`tasks`.`collaborators` AS `collaborators`', 'dt' => count($columns), 'field' => 'collaborators' ));
    array_push($columns, array( 'db' => '`tasks`.`status` AS `status`', 'dt' => count($columns), 'field' => 'status', 'formatter' => function($d, $row) {
                $clr = 'a49ae0';
                if($d == 'to_do') {
                    $clr = 'f77d73';
            ...
            array_push($columns, array( 'db' => 'tasks.id AS task_id', 'field' => 'task_id', 'dt' => count($columns), 'formatter' => function($d, $row){
                $t = '';

                if(CheckPermission('tasks', "all_update")){
                ...
                }else if(CheckPermission('tasks', "own_update") && (CheckPermission('tasks', "all_update")!=true)){
                    $user_id =getRowByTableColomId('tasks',$d,'id');
                    print_r($user_id);die();
                ...
                if(CheckPermission('tasks', "all_delete")){
                $t .= '<a data-toggle="modal" class="mClass" style="cursor:pointer;"  data-target="#cnfrm_delete" title="'.lang('delete').'" onclick="setId('.$d.', \'tasks\')"><i class="material-icons col-red font-20">delete</i></a>';
                }else if(CheckPermission('tasks', "own_delete") && (CheckPermission('tasks', "all_delete")!=true)){
                    $user_id =getRowByTableColomId('tasks',$d,'id');
                    if($user_id->user_id==$this->user_id){
                $t .= '<a data-toggle="modal" class="mClass" style="cursor:pointer;"  data-target="#cnfrm_delete" title="'.lang('delete').'" onclick="setId('.$id.', \'tasks\')"><i class="material-icons col-red font-20">delete</i></a>';
                    }
                }

                $t .= '<a data-toggle="modal" href="'.base_url().'tasks/view/'.$d.'" class="mClass" style="cursor:pointer;" title="View Ticket"><i class="material-icons font-20">remove_red_eye</i></a>';
                return $t;
            } ));  
        $where = '';
        $j = 0;
...
*/
    ...
    public function getFilterdata(){
        ...
        $data["view_data"]= $this->Tasks_model->getData($where);
    ...
    public function getGridInfoBoxVal() {
        ...
        if(is_array($this->input->get('request')) && !empty($this->input->get('request'))) {
            foreach ($this->input->get('request') as $rkey => $rvalue) {
                ...
                $res = $this->Tasks_model->getQrResult($qr);
    ...
    public function recurring() {
        ...
        $recurring_expenses = $this->Tasks_model->getDataBy($table, 1, 'recurring');
        ...
        foreach ($recurring_expenses as $expense) {
            ...
            if (date('Y-m-d') >= $calculated_date_difference) {
                ...
                $tk_data = $this->Tasks_model->getDataBy('ticket', $expense['ticket_id'], 'ticket_id');
                $u_id = $tk_data[0]->ticket_client;
                if($this->session->get_userdata()['user_details'][0]->ia_users_id == $u_id) {
                    $u_id = $tk_data[0]->user_id;
                }
                //$user_detail = $this->Tasks_model->get_email_tk($u_ud);
                $user_detail = $this->Tasks_model->getDataBy('ia_users', $u_id, 'ia_users_id');
                ...
                if($user_detail[0]->email != '') {
                    ...
                    if($setting['mail_setting'] == 'php_mailer') {
                        $this->load->library("send_mail");         
                        $emm = $this->send_mail->email($sub, $body, $email, $setting);
                        $this->Tasks_model->updateRow('ticket', 'ticket_id', $expense['ticket_id'], array('last_recurring_date' => date('Y-m-d')));
                    } else {
                        // content-type is required when sending HTML email
                        $headers = "MIME-Version: 1.0" . "\r\n";
                        $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
