...
<div class="col-md-1">
        		<span class="mka-com-add DeshboardModal" data-crud-id="63353"></span>
        	</div>
        </div>
...
var data_table = function($filter, $search) {
	...
	return table = $("#example_customers").DataTable({
		"dom": 'lfBrtip',
   ...
   $(".daterange-picker").daterangepicker(
    {
		...
		<?php if(!isset($_COOKIE['customers_filter']) || $_COOKIE['customers_filter'] == '') { ?>
        ,
...
jQuery(document).ready(function() {
	get_grid_info_box_val('customers');
	...
	$("body").on("change", '.filter-field', function() {
        ...
        get_grid_info_box_val('customers');
        $.post('<?php echo base_url().'invoice/set_filter_cookie' ?>', {customers_filter: $filter});
    });
	...
	$('body').on('submit', '#form', function(ev) {
		ev.preventDefault();
		$('#nameModal_customers').find('input[name="save"]').prop('disabled', true);
		...
		$.ajax({
			url: '<?php echo base_url().'invoice/add_edit_customers' ?>',
			method: 'POST',
			async: false,
			data: formData,
			cache: false,
			...
			if(mka > 0) {
				...
				setTimeout(function() {
					...
					$.post('<?php echo base_url().'invoice/get_filter_html/1' ?>', function(data) {
...
$(document).ready(function(){
	...
	setTimeout(function() {
		...
		if(cjhk == 1) {
			$('.dataTables_info').before('<button data-del-url="<?php echo base_url() ?>invoice/delete_customers/" rel="delSelTable" class="btn btn-default btn-sm delSelected pull-left"> <i class="material-icons col-red">delete</i> </button><br><br>');
		}
	}, 1000);
