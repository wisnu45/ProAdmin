...
class Document_maker_model extends CI_Model {       
	...
	public function getDataId($id='') {
		 $this->db->select('*');
		 $this->db->from('ia_document_maker');
	...
	public function getData($con=NULL) {
		if(CheckPermission('ia_document_maker', "own_read") && CheckPermission('ia_document_maker', "all_read")!=true){
			...
			$con .= "  (`ia_document_maker`.`user_id` = '".$this->user_id."') ";
		}
		$sql = "SELECT * FROM  `ia_document_maker` ";
		if($con != '') {
			$sql .= ' WHERE '.$con;	
		}
