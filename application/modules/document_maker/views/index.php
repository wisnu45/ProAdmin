		...
		<!-- Main content -->
            <div class="row">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
						<div class="header">
<h2><?php echo lang('document_maker_menuForm'); ?> <div align="center" style="width:60%;float: right;text-align: -webkit-left;"><?php echo lang('document_makerTitle') ?></div></h2>

  <ul class="header-dropdown">
     <?php if(CheckPermission("document_maker", "own_create")){ ?>
     <button type="button" class="btn-sm  btn btn-primary waves-effect amDisable modalButton" data-src="" data-width="555" ><i class="material-icons">add</i><?php echo lang('add_Form_maker') ?></button>
  <?php }?>
  </ul>
</div>
                 	 <!-- /.box-header -->
...
var data_table = function($filter, $search) {
	...
	return table = $("#example_document_maker").DataTable({
        ...
        "serverSide": true,
        "ajax": {
        	"url" : url + "document_maker/ajxData",
				"data": function ( d ) {
					console.log('data->>',d);
					if($filter != '') {
   ...
   $(".daterange-picker").daterangepicker(
    {
		...
		<?php if(!isset($_COOKIE['document_maker_filter']) || $_COOKIE['document_maker_filter'] == '') { ?>
        ,
...
jQuery(document).ready(function() {
	...
	$("body").on("change", '.filter-field', function() {
        ...
        //getGridInfoBoxVal('document_maker');
        $.post('<?php echo base_url().'document_maker/setFilterCookie' ?>', {document_maker_filter: $filter});
    });
	...
	$('body').on('submit', '#form', function(ev) {
		...
		$.ajax({
			url: '<?php echo base_url().'document_maker/addedit' ?>',
			method: 'POST',
			async: false,
			data: formData,
			cache: false,
			...
			console.log(mka);
			if(mka > 0 && $('input[name="id"]').length > 0) {
				...
				setTimeout(function() {
					...
					$.post('<?php echo base_url().'document_maker/getFilterHtml/1' ?>', function(data) {
				...
				window.location.href = $('body').attr('data-base-url') + 'document_maker/formBuider/' + mka;
			}
		});
	});
...
$(document).ready(function(){
	...
	$("body").on("click",".modalButton", function(e) {  
		...
		$.ajax({
			url : "<?php echo base_url()."document_maker/getmodal";?>",
			method: "post", 
			data : {
...
if(document_maker_titlea !=''){
	$("#titleForm").html('<?php echo lang('edit_maker'); ?>');
} else{
	$("#titleForm").html('<?php echo lang('add_maker'); ?>');
}
