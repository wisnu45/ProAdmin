...
class SSP {
		...
		for ($i = 0, $ien = count($data); $i < $ien; $i++) {
			...
			for ($j = 0, $jen = count($columns); $j < $jen; $j++) {
				...
				if (isset($column['formatter'])) {
					...
					if (isset($data[$i][$columns[$j]['db']])) {
						$row[$column['dt']] = ($isJoin) ? $data[$i][$columns[$j]['field']] : $data[$i][$columns[$j]['db']];
					} else {
						$row[$column['dt']] = ($isJoin) ? $data[$i][$columns[$j]['field']] : $data[$i];
					}
					// print_r($data);die();
	...
	static function iaDataOutput($columns, $data, $isJoin = false) {
		$out = array();
	...
	static function iaOrder($request, $columns, $isJoin = false) {
		$order = '';
	...
	static function iaFilter($request, $columns, $isJoin = false) {
		$globalSearch = array();
		$columnSearch = array();
		$dtColumns = SSP::pluck($columns, 'dt');
		if (isset($request['search']) && $request['search']['value'] != '') {
	...
	static function column_ia_filter($request, $where = '') {
